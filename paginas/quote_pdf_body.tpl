<html>

<head>
    <meta http-equiv=Content-Type content="text/html; charset=UTF-8">

    <style type="text/css">
        span.cls_003 {
            font-family: "Cambria Bold", serif;
            font-size: 25px;
            color: rgb(54, 95, 146);
            font-weight: bold;
            font-style: normal;
            text-decoration: none
        }

        div.cls_003 {
            font-family: "Cambria Bold", serif;
            font-size: 25px;
            color: rgb(54, 95, 146);
            font-weight: bold;
            font-style: normal;
            text-decoration: none
        }

        span.cls_004 {
            font-family: "Cambria Bold", serif;
            font-size: 19px;
            color: rgb(148, 179, 214);
            font-weight: bold;
            font-style: normal;
            text-decoration: none
        }

        div.cls_004 {
            font-family: "Cambria Bold", serif;
            font-size: 19px;
            color: rgb(148, 179, 214);
            font-weight: bold;
            font-style: normal;
            text-decoration: none
        }

        span.cls_005 {
            font-family: "Calibri", serif;
            font-size: 12px;
            color: rgb(0, 0, 0);
            font-weight: normal;
            font-style: normal;
            text-decoration: none
        }

        div.cls_005 {
            font-family: "Calibri", serif;
            font-size: 12px;
            color: rgb(0, 0, 0);
            font-weight: normal;
            font-style: normal;
            text-decoration: none
        }

        span.cls_002 {
            font-family: "Cambria Bold", serif;
            font-size: 15px;
            color: rgb(255, 255, 255);
            font-weight: bold;
            font-style: normal;
            text-decoration: none
        }

        div.cls_002 {
            font-family: "Cambria Bold", serif;
            font-size: 15px;
            color: rgb(255, 255, 255);
            font-weight: bold;
            font-style: normal;
            text-decoration: none
        }

        span.cls_011 {
            font-family: "Calibri Bold Italic", serif;
            font-size: 18px;
            color: rgb(0, 0, 0);
            font-weight: bold;
            font-style: italic;
            text-decoration: none
        }

        div.cls_011 {
            font-family: "Calibri Bold Italic", serif;
            font-size: 18px;
            color: rgb(0, 0, 0);
            font-weight: bold;
            font-style: italic;
            text-decoration: none
        }

        span.cls_010 {
            font-family: "Calibri Bold", serif;
            font-size: 11.6px;
            color: rgb(0, 0, 0);
            font-weight: bold;
            font-style: normal;
            text-decoration: none
        }

        div.cls_010 {
            font-family: "Calibri Bold", serif;
            font-size: 11.6px;
            color: rgb(0, 0, 0);
            font-weight: bold;
            font-style: normal;
            text-decoration: none
        }
    </style>
</head>

<body>

    <table border="0" width="100%" cellSpacing="0" cellPadding="2" style="line-height:20px;">
        <tr>
            <td colspan="3">
                <div class="cls_003"><span class="cls_003">HUBROX TECHNOLOGY INC.</span></div>
            </td>
            <td colspan="2" align="center">
                <div class="cls_004"><span class="cls_004">[var.format;noerr]</span>
                </div>
            </td>
        </tr>
        <tr>
            <td width="49%" colspan="2">
                <div class="cls_005"><span class="cls_005">325 Autumn Hill Blvd</span></div>
            </td>
            <td width="17%"></td>
            <td width="17%"></td>
            <td width="17%"></td>
        </tr>
        <tr>
            <td colspan="2">
                <div class="cls_005"><span class="cls_005">Thornhill, ON, I4J8Y4, Canada</span></div>
            </td>
            <td></td>
            <td>
                <div class="cls_005"><span class="cls_005">DATE</span></div>
            </td>
            <td>
                <div class="cls_005"><span class="cls_005"></span>[var.date;noerr]</span></div>
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <div class="cls_005"><span class="cls_005">Website: www.hubrox.com</span></div>
            </td>
            <td></td>
            <td>
                <div class="cls_005"><span class="cls_005">INVOICE #</span></div>
            </td>
            <td>
                <div class="cls_005"><span class="cls_005">[var.quoteNumber;noerr]</span></div>
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <div class="cls_005"><span class="cls_005">Phone: +1 647-4995741</span></div>
            </td>
            <td></td>
            <td>
                <div class="cls_005"><span class="cls_005">PO#</span></div>
            </td>
            <td></td>
        </tr>
        <tr>
            <td colspan="2">
                <div class="cls_005"><span class="cls_005">Contact: Alison Xia</span></div>
            </td>
            <td></td>
            <td>
                <div class="cls_005"><span class="cls_005">SALES PERSON</span>
            </td>
            <td>
                <div class="cls_005"><span class="cls_005">[var.prepared;noerr]</span>
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <div class="cls_005"><span class="cls_005">E-mail: sales@hubrox.com</span></div>
            </td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr>
            <td colspan="5">
                <div class="cls_005"><span class="cls_005">&nbsp;</span></div>
            </td>
        </tr>
        <tr>
            <td colspan="3" bgcolor="rgb(79, 129, 189)">
                <div class="cls_002"><span class="cls_002">BILL TO</span></div>
            </td>
            <td colspan="2" bgcolor="rgb(79, 129, 189)">
                <div class="cls_002"><span class="cls_002">SHIP TO (it different)</span>
                </div>
            </td>
        </tr>
        <tr>
            <td>
                <div class="cls_005"><span class="cls_005">[Name]</span></div>
            </td>
            <td colspan="2">
                <div class="cls_005"><span class="cls_005">[var.contactName;noerr].</span></div>
            </td>
            <td colspan="2" rowspan="7" valign="top">
                <div class="cls_005"><span class="cls_005">[var.shipTo;noerr].</span></div>
            </td>
        </tr>
        <tr>
            <td>
                <div class="cls_005"><span class="cls_005">[Company Name]</span></div>
            </td>
            <td colspan="2">
                <div class="cls_005"><span class="cls_005">[var.orgName;noerr].</span></div>
            </td>
        </tr>
        <tr>
            <td>
                <div class="cls_005"><span class="cls_005">[Street Address]</span></div>
            </td>
            <td colspan="2">
                <div class="cls_005"><span class="cls_005">[var.orgAddress;noerr]</span></div>
            </td>
        </tr>
        <tr>
            <td>
                <div class="cls_005"><span class="cls_005">[City, ST, ZIP]</span></div>
            </td>
            <td colspan="2">
                <div class="cls_005"><span class="cls_005">[var.orgCity;noerr], [var.orgCountry;noerr].</span></div>
            </td>
        </tr>
        <tr>
            <td>
                <div class="cls_005"><span class="cls_005">[Phone]</span></div>
            </td>
            <td colspan="2">
                <div class="cls_005"><span class="cls_005">[var.orgPhone;noerr]</span></div>
            </td>
        </tr>
        <tr>
            <td>
                <div class="cls_005"><span class="cls_005">[E-mail]</span></div>
            </td>
            <td colspan="2">
                <div class="cls_005"><span class="cls_005">[var.orgEmail;noerr]</span></div>
            </td>
        </tr>
        <tr>
            <td>
                <div class="cls_005"><span class="cls_005">[Website]</span></div>
            </td>
            <td colspan="2">
                <div class="cls_005"><span class="cls_005">[var.orgWeb;noerr]</span></div>
            </td>
        </tr>
        <tr>
            <td colspan="5">
                <div class="cls_005"><span class="cls_005">&nbsp;</span></div>
            </td>
        </tr>
        <tr>
            <td colspan="5">
                <div class="cls_005"><span class="cls_005">&nbsp;</span></div>
            </td>
        </tr>
        <tr>
            <td colspan="2" bgcolor="rgb(79, 129, 189)">
                <div class="cls_002"><span class="cls_002">DESCRIPTION</span></div>
            </td>
            <td bgcolor="rgb(79, 129, 189)" align="center">
                <div class="cls_002"><span class="cls_002">UNIT PRICE</span>
                </div>
            </td>
            <td bgcolor="rgb(79, 129, 189)" align="center">
                <div class="cls_002"><span class="cls_002">QTY</span>
                </div>
            </td>
            <td bgcolor="rgb(79, 129, 189)" align="center">
                <div class="cls_002"><span class="cls_002">AMOUNT ([var.currency;noerr])</span>
                </div>
            </td>
        </tr>
        <tr bgcolor="rgb([products.line_color;block=tr;noerr], [products.line_color;noerr], [products.line_color;noerr])">
            <td colspan="2">
                <div class="cls_010"><span class="cls_010">[products.desc;noerr]</span></div>
            </td>
            <td align="center">
                <div class="cls_005"><span class="cls_005">[products.price;noerr]</span>
                </div>
            </td>
            <td align="center">
                <div class="cls_005"><span class="cls_005">[products.qty;noerr]</span>
                </div>
            </td>
            <td align="center">
                <div class="cls_005"><span class="cls_005">[products.amount;noerr]</span>
                </div>
            </td>
        </tr>
        <tr>
            <td colspan="3">
                <div class="cls_005"><span class="cls_005">&nbsp;</span></div>
            </td>
            <td align="right">
                <div class="cls_010"><span class="cls_010">TOTAL ([var.currency;noerr])</span></div>
            </td>
            <td align="center">
                <div class="cls_005"><span class="cls_005">[var.sunTotal;noerr]</span></div>
            </td>
        </tr>
        <tr>
            <td colspan="3" bgcolor="rgb(79, 129, 189)">
                <div class="cls_002"><span class="cls_002">TERMS AND CONDITIONS</span></div>
            </td>
            <td align="right">
                <div class="cls_010"><span class="cls_010">CIP MIAMI</span></div>
            </td>
            <td>
                <div class="cls_005"><span class="cls_005">&nbsp;</span></div>
            </td>
        </tr>
        <tr>
            <td colspan="3" nowrap>
                <div class="cls_005"><span class="cls_005">1. Payment will be due prior to delivery of service and
                        goods<br>
                        2. Customers are responsible for import duties and<br>brokerage fees if applied during the
                        shipment<br>
                        3. CIP MIAMI
                    </span></div>
            </td>
            <td colspan="2">
                <div class="cls_005"><span class="cls_005">&nbsp;</span></div>
            </td>
        </tr>
        <tr>
            <td colspan="5">
                <div class="cls_005"><span class="cls_005">&nbsp;</span></div>
            </td>
        </tr>
        <tr>
            <td colspan="5">
                <!-- <div class="cls_010"><span class="cls_010">HST#: 822913786RT0001 </span>
                </div> -->
            </td>
        </tr>
        <tr>
            <td colspan="5">
                <div class="cls_005"><span class="cls_005">&nbsp;</span></div>
            </td>
        </tr>
        <tr>
            <td colspan="5">
                <div class="cls_005"><span class="cls_005">&nbsp;</span></div>
            </td>
        </tr>
        <tr>
            <td colspan="5">
                <div class="cls_005"><span class="cls_005">&nbsp;</span></div>
            </td>
        </tr>
        <tr>
            <td colspan="5" align="center">
                <div class="cls_011"><span class="cls_011">Thank You For Your Business!</span></div>
            </td>
        </tr>

    </table>
</body>

</html>